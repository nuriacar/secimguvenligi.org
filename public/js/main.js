  ...
  async function search(q) {
    ...
    if (scored.length === 0) {
      ...
      for (var i = 0; i < scored.length && shown < 8; i++) {
        ...
        a.innerHTML = '<div class="search-result-title">' + p.title + '</div><div class="search-result-excerpt">' + excerpt(p.content, q) + '</div>';
...
(function () {
  ...
  if (toggle && links) {
    toggle.addEventListener('click', function () {
      ...
      toggle.setAttribute('aria-label', links.classList.contains('open') ? 'Menüyü kapa' : 'Menüyü aç/kapa');
    });
    document.addEventListener('click', function (e) {
...
(function () {
  ...
  function setFontSize(size) {
    ...
    if (btnShrink && btnReset && btnGrow) {
      [btnShrink, btnReset, btnGrow].forEach(function (b) { b.className = 'font-btn'; });
